...
def compute_global_pixel_accuracy(preds, labels):
    """Return (correct_pixels, total_pixels) for global accuracy aggregation."""
...
def update_per_class_counts(preds, labels, class_correct, class_total, num_classes=NUM_CLASSES):
    """Accumulate per-class correct/total counts (for balanced accuracy)."""
...
def compute_balanced_accuracy_from_counts(class_correct, class_total):
    """Mean of class-wise accuracies: (Acc0 + Acc1 + ...)/N_present."""
    accs = []
...
for epoch in range(NUM_EPOCHS):
    ...
    # ---- Validation (with global & balanced accuracy) ----
    model.eval()
...
# 8. VISUALIZATION
# =====================
output_dir = "unet_output_images"
os.makedirs(output_dir, exist_ok=True)

# Safer iteration in case test_loader is empty
try:
